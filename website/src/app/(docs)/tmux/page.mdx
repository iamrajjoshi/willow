...
Willow includes native tmux support — a worktree picker popup, live agent output preview, status bar widget, and automatic session management. If you use tmux, this replaces the need for any separate worktree-switching plugin.
...
| `Ctrl-G` | Dispatch: create worktree from query text as prompt, launch `agent.default` |
| `Ctrl-O` | Dispatch with a one-off agent harness picker |
...
It also tracks state transitions — when an agent session goes from BUSY to any other state, it triggers an audio notification (macOS `Glass.aiff` by default).
